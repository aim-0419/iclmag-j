...
npm ci
npx prisma generate
npx prisma db push  # 데이터베이스 구조를 스키마와 맞춤
npm run build       # 빌드가 실패하면 여기서 멈춤
pm2 restart iclmag-j
```

> **`set -e` 가 하는 일**
> 빌드나 DB 반영이 실패하면 `pm2 restart` 까지 가지 않고 멈춥니다.
> 즉 배포가 실패해도 **기존에 돌아가던 사이트는 그대로 유지**됩니다.

> **`prisma db push` 가 필요한 이유**
> `prisma/schema.prisma` 에 표(테이블)를 추가해도, 데이터베이스에 실제로 만들어 주지 않으면
> 그 기능은 동작하지 않습니다. 실제로 비밀번호 재설정용 표가 서버에 만들어지지 않아
> **비밀번호 찾기가 동작하지 않던 문제**가 있었습니다. 이 단계가 그것을 막아 줍니다.
>
> 기존 데이터가 지워질 수 있는 변경이면 이 명령이 실패하고 배포가 멈춥니다.
> (`--accept-data-loss` 를 일부러 쓰지 않았습니다.) 그때는 사람이 직접 확인해야 합니다.
...
| 증상 | 원인 | 해결 |
|------|------|------|
| 서버가 시작되지 않고 로그에 `[보안 설정 오류]` | `JWT_SECRET` 미설정 또는 공개된 예시값 | 아래 명령으로 새 열쇠를 만들어 `.env.local` 에 넣고 재시작 |
| 비밀번호 찾기에서 오류(500) | 데이터베이스에 `password_reset_tokens` 표가 없음 | `npm run db:push` 실행 |
| 로그인은 되는데 새로고침하면 풀림 | https 주소인데 `NEXT_PUBLIC_APP_URL` 이 http 로 되어 있음 (또는 그 반대) | `.env.local` 의 주소를 실제 접속 주소와 맞춤 |
| 인증 메일이 오지 않음 | `EMAIL_*` 설정 누락 또는 Gmail 앱 비밀번호 미발급 | `.env.local` 확인 (가입 화면의 "인증 코드 다시 받기"로 재시도 가능) |
| 화면에 기사가 안 보임 | 임시저장(DRAFT) 상태이거나, 홈 화면 갱신 대기 중 | 기사를 발행 상태로 저장 / 최대 1분 대기 |
| `시도 횟수가 너무 많습니다` | 반복 시도 차단에 걸림 | 안내된 시간만큼 대기 (서버 재시작 시 초기화) |
...
### 데이터베이스 구조가 코드와 맞는지 확인하는 법

```bash
npm run db:studio   # 브라우저로 표 목록 확인
```

아래 4개 표가 모두 있어야 합니다.

`users` · `articles` · `verification_tokens` · `password_reset_tokens`

하나라도 없으면 `npm run db:push` 를 실행하세요.
